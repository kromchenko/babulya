...
class DCE_Ajax {
    ...
    function init() {
        ...
        add_action( 'wp_ajax_nopriv_wpa_dce_file_browser_hits', array($this, 'wpa_dce_file_browser_hits') );
    ...
    function wpa_dce_file_browser_hits() {
        // The $_REQUEST contains all the data sent via ajax
        if ( isset($_REQUEST) ) {
            if (isset($_REQUEST['post_id'])) {
                $post_id = $_REQUEST['post_id'];
    ...
    public static function dce_ajax_action() {
        ...
        $this->get_template_for_ajax($template_id, $postid);
    ...
    private function get_template_for_ajax($t_id, $p_id) {
        if ($t_id > 0) {
