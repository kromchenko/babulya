...
class DCE_Widget_FileBrowser extends DCE_Widget_Prototype {
    ...
    protected function render() {
        ...
        if( $settings['folder'] != '' ) {
            ...
            //var_dump($baseDir);
            if (is_dir($this->getRootDir($baseDir))) {
                ...
                if ($settings['title']) { echo '<h4>'.$baseTitle.'</h4>'; }
                ...
                echo '>';
                    $this->dirToHtml($this->getRootDir($baseDir));
                echo '</ul>';
    ...
    public static function getRootDir($folder = false) {
        $dir = wp_upload_dir();
        $dir = $dir["basedir"];
        if ($folder) {
    ...
    public function dirToHtml($dir, $hidden = false) {
        ...
        $cdir = scandir($dir, $settings['order']);
        foreach ($cdir as $key => $value) {
            if (substr($value, 0, 1) != '.') { // hidden file
                ...
                $rdir = str_replace($this->getRootDir(), '', $fulldir);
                if (is_dir($fulldir)) {
                    $hide = false;
                    ...
                    if ( \Elementor\Plugin::$instance->editor->is_edit_mode() || !$hide) {
                        ...
                        echo '<a class="'.($customTitle ? 'inline-' : '').'block btn-block dce-file-download" href="'.DCE_Helper::path_to_url($fulldir).'"  data-md5="'.$md5.'"'.($post_id ? ' data-post-id="'.$post_id.'"' : '').' target="_blank">';
                            ...
                            if ($settings['enable_metadata'] && $settings['enable_metadata_size']) {
                                echo ' <small class="label label-default dce-file-size-label">('.$this->humanFilesize(filesize($fulldir), 0).')</small>';
                            }

                            if ($settings['enable_metadata'] && $settings['enable_metadata_hits']) {
