...
class DCE_Widget_Prototype extends Widget_Base {
    ...
    public function satisfy_dependencies($ret = false, $deps = array()) {
        ...
        //var_dump((array)get_option( 'active_plugins', array()));
        $depsDisabled = array();
        if (!empty($deps)) {
            $active_plugins = (array)get_option( 'active_plugins', array());
            foreach ($deps as $pkey => $plugin) {
                $pluginFile = $plugin.'.php';
                $checkPlugin = $pkey.'/'.$pluginFile;
                $isActive = in_array($checkPlugin, $active_plugins);
                /*if (!$isActive) {
                    foreach ($active_plugins as $aplugin) {
                        $pieces = explode('/', $aplugin);
                        if (end($pieces) == $pluginFile) {
                            $isActive = true;
                        }
                    }
                }*/
                if (!$isActive) { // probably only for ACF Pro
                    foreach ($active_plugins as $aplugin) {
                        $pieces = explode('/', $aplugin);
                        if (strpos(reset($pieces), $pkey) !== false) {
                            $isActive = true;
                        }
                    }
                }
                //$isActive = is_plugin_active($checkPlugin);
