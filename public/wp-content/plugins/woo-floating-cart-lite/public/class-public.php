...
class XT_Woo_Floating_Cart_Public
{
    ...
    public function define_woocommerce_constants()
    {
        ...
        if ( $this->enabled() && !defined( 'WOOCOMMERCE_CART' ) ) {
            define( 'WOOCOMMERCE_CART', true );
            if ( $this->core->fs()->can_use_premium_code__premium_only() && xt_woofc_option_bool( 'cart_checkout_form', false ) && !defined( 'WOOCOMMERCE_CHECKOUT' ) ) {
    ...
    public function enqueue_vendors()
    {
        ...
        wp_enqueue_script(
            'xt-tweenmax',
            $this->core->plugin_url( 'public' ) . 'assets/vendors/TweenMax' . $this->core->script_suffix . '.js',
            array( 'jquery' ),
            $this->core->plugin_version(),
            false
        );
